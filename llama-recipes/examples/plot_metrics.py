...
def plot_metric(data, metric_name, x_label, y_label, title, colors):
    ...
    plt.plot(data[f'train_{metric_name}'], label=f'Train Epoch {metric_name.capitalize()}', color=colors[0])
    plt.plot(data[f'val_{metric_name}'], label=f'Validation Epoch {metric_name.capitalize()}', color=colors[1])
    plt.xlabel(x_label)
    plt.ylabel(y_label)
