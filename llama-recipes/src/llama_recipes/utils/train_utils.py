...
def train(
    ...
    Args:
        ...
        alpaca_dataloader: The dataloader containing the alpaca data
    ...
    for epoch in range(train_config.num_epochs):
        ...
        if train_config.run_validation:
            ...
            (
                ...
                alpaca_epoch_loss,
                temp_alpaca_loss,
                temp_alpaca_perplexity,
            ) = evaluation(
                model, train_config, alpaca_dataloader, local_rank, tokenizer
            )
            if train_config.save_metrics:
                val_step_loss.extend(temp_val_loss)
                val_step_perplexity.extend(temp_step_perplexity)
                alpaca_step_loss.extend(temp_alpaca_loss)
                alpaca_step_perplexity.extend(temp_alpaca_perplexity)
            ...
            if train_config.save_model:  # and eval_epoch_loss < best_val_loss
                ...
                if train_config.use_peft:
                    if train_config.enable_fsdp:
                        ...
                        print(f"we are about to save the PEFT modules")
                    model.save_pretrained(epoch_out_dir)
                    if train_config.enable_fsdp:
                        if rank == 0:
                            print(
                                f"PEFT modules are saved in {epoch_out_dir} directory"
                            )
                    else:
                        print(f"PEFT modules are saved in {epoch_out_dir} directory")
            ...
            if alpaca_epoch_loss < best_alpaca_loss:
                best_alpaca_loss = alpaca_epoch_loss
                if train_config.enable_fsdp:
                    if rank == 0:
                        print(
                            f"best alpaca loss on epoch {epoch+1} is {best_alpaca_loss}"
                        )
                else:
                    print(f"best alpaca loss on epoch {epoch+1} is {best_alpaca_loss}")
            alpaca_loss.append(float(best_alpaca_loss))
            alpaca_prep.append(float(alpaca_ppl))
        if train_config.enable_fsdp:
            if rank == 0:
                print(
    ...
    if train_config.run_validation:
        ...
        avg_alpaca_prep = sum(alpaca_prep) / len(alpaca_prep)
        avg_alpaca_loss = sum(alpaca_loss) / len(alpaca_loss)
    ...
    if train_config.run_validation:
        ...
        results["avg_alpaca_prep"] = avg_alpaca_prep
        results["avg_alpaca_loss"] = avg_alpaca_loss
